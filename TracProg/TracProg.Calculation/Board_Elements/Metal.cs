...
{
    public class Metal : IBoardElement
    {
        private Rectangle _rect;
        private Color _color = Color.FromArgb(183, 65, 14);
        ...
        public Metal(string ID, int x, int y, int width, int height)
        {
        ...
        {
            graphics.FillRectangle(new SolidBrush(Color), (X + 0.5f) + (((Width - 1) - ((Width - 1) / 2.0f))) / 2.0f, (Y + 0.5f) + (((Height - 1) - ((Height - 1) / 2.0f))) / 2.0f, (Width - 1) / 2.0f, (Height - 1) / 2.0f);
        }
